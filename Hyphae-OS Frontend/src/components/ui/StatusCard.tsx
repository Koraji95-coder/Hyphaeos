...
const StatusCard: React.FC<StatusCardProps> = ({
  ...
  return (
    <motion.div 
      className="bg-dark-200/80 backdrop-blur-sm rounded-xl border border-dark-100/50 shadow-lg p-5"
      variants={variants}
      whileHover={{ y: -4, transition: { duration: 0.2 } }}
    >
      <div className="flex items-center justify-between mb-4">
        <div className={`p-2 rounded-lg ${getColorClass('bg')}`}>
          {React.cloneElement(icon as React.ReactElement, { className: getColorClass('text') })}
        </div>
        <div className="flex items-center">
          {getTrendIcon()}
          <span className={`ml-1 text-xs ${getTrendClass()}`}>{trendValue}</span>
        </div>
      </div>
      <div>
