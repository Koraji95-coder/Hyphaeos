...
const Navigation: React.FC<NavigationProps> = ({ 
  ...
  return (
    <>
                  ...
                  <div className="flex items-center">
                    {user.avatar ? (
                      ...
                      <div className="w-10 h-10 rounded-full bg-primary-500/20 flex items-center justify-center mr-3">
                        <span className="text-primary-300 font-medium">{user.username.charAt(0).toUpperCase()}</span>
                      </div>
                    )}
                    <div>
      ...
      >
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'px-6'} h-16 border-b border-dark-100/50`}>
          {isCollapsed ? (
            <motion.div 
          ...
          <nav className={`space-y-1 ${isCollapsed ? 'px-3' : 'px-4'}`}>
            {navigationItems.map((item) => (
              ...
              >
                ...
                {isCollapsed && <span className="text-xs">{item.label.split(' ')[0]}</span>}
              </motion.button>
            ))}
          </nav>
        </div>
        
        <div className={`p-4 border-t border-dark-100/50 ${isCollapsed ? 'items-center justify-center' : ''}`}>
          {user && !isCollapsed && (
