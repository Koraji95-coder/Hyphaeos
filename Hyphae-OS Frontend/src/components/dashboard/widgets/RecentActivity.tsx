...
import { ArrowUpRight, MessageSquare, Zap, RefreshCw, FileText, Database } from 'lucide-react';
          ...
          {activityData.map((activity, index) => (
            ...
            >
              <div className="relative mr-4 flex-shrink-0">
                <div className={`w-8 h-8 rounded-full ${getAgentColor(activity.color)} flex items-center justify-center`}>
                  {activity.icon}
                </div>
                ...
                <div className="flex items-start justify-between">
                  <div>
                    <p className="text-sm font-medium text-white">
                      <span className={`text-${activity.color}-400`}>{activity.agent}</span> {activity.action}
                    </p>
