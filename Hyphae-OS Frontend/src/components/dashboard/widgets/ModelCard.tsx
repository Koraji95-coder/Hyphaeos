...
import { ArrowUpRight, RefreshCw, TrendingUp, Minus, TrendingDown } from 'lucide-react';
      ...
      <div className="p-5">
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <span className="text-xs text-gray-400">{model.type} Model</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          <div className="bg-dark-300/50 rounded-lg p-3 border border-dark-100/30">
            <div className="flex justify-between items-center mb-1">
              ...
              <div className="flex items-center text-xs">
                {getTrendIcon()}
                <span className={`
                  ${model.trend === 'up' ? 'text-success-400' : 
                    model.trend === 'down' ? 'text-error-400' : 'text-gray-400'}
                `}>
                  {model.trend === 'up' ? '+1.2%' : 
                   model.trend === 'down' ? '-0.8%' : '0%'}
                </span>
              </div>
            </div>
            ...
            <div className="w-full bg-dark-100/50 rounded-full h-1.5 mt-2">
              <div 
                className={`h-1.5 rounded-full ${
                  model.accuracy > 95 ? 'bg-success-500' : 
                  model.accuracy > 85 ? 'bg-primary-500' : 
                  model.accuracy > 75 ? 'bg-warning-500' : 'bg-error-500'
                }`}
          ...
          <div className="bg-dark-300/50 rounded-lg p-3 border border-dark-100/30">
            ...
            <div className="mt-2 text-xs text-gray-500">Last trained {model.lastTrained}</div>
          </div>
        </div>
          ...
          <span className="text-xs text-gray-400">
            {model.status === 'training' ? (
              <span className="flex items-center">
                <svg className="animate-spin -ml-1 mr-2 h-3 w-3 text-warning-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
